...
public abstract class Sorter {
	 ...
	 */
	public void sortWithStatistics(WordList toSort, Comparator<String> comp, int totalToSort) throws NullPointerException, IllegalArgumentException, FileNotFoundException {
		String fileName = this.getClass().getSimpleName() + ".txt";
		PrintWriter pw = new PrintWriter(fileName);
		WordList x = toSort.clone();
		CountingComparator<String> cc = new CountingComparator<>(comp);
		while (totalWordsSorted < totalToSort) {
			long start = System.currentTimeMillis();
			this.sort(x, cc);
			long end = System.currentTimeMillis();
			totalWordsSorted += x.length();
